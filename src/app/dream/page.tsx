  ...
  if (selectedDream) {
    return (
      <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-right-4 duration-500 pb-20">
        ...
        <div className="card-minimal space-y-8 border-t-8 border-t-blue-600 p-8 shadow-card">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="space-y-2">
              <span className="px-3 py-1 bg-blue-50 text-blue-600 rounded-full text-[10px] font-black uppercase tracking-widest">
                {selectedDream.category}
              </span>
              <h1 className="text-5xl font-black text-slate-900 tracking-tight">{selectedDream.title}</h1>
            </div>
            
            <div className="flex flex-wrap gap-2 justify-end">
              {selectedDream.luckyNumbers.map((num, idx) => (
                <div key={idx} className="bg-gradient-to-br from-blue-600 to-indigo-700 text-white rounded-2xl p-4 min-w-[80px] text-center shadow-md shadow-blue-200">
                  <div className="text-2xl font-mono font-black">{num}</div>
          ...
          <div className="p-8 bg-blue-50/50 rounded-3xl border border-blue-100/50 relative overflow-hidden">
             <Sparkles className="absolute top-[-10px] right-[-10px] w-24 h-24 text-blue-100/30" />
            <h3 className="text-sm font-black text-blue-400 uppercase mb-4 tracking-widest flex items-center gap-2">
        ...
        <div className="space-y-6">
          <div className="flex flex-col space-y-2">
             <div className="flex items-center space-x-2 text-slate-800">
              ...
              <h2 className="text-2xl font-black">วิเคราะห์ความเป็นไปได้</h2>
            </div>
                  ...
                  <div key={idx} className={`card-minimal flex flex-col md:flex-row md:items-center justify-between gap-6 transition-all border-l-8 ${isHot ? 'border-l-green-500 bg-white' : 'border-l-slate-200 bg-slate-50/50 grayscale-[0.5]'} hover:shadow-card p-6`}>
                    <div className="flex items-center space-x-8">
                      <div className="flex flex-col items-center">
                        <div className={`text-4xl font-mono font-black ${isHot ? 'text-blue-600' : 'text-slate-400'}`}>{num}</div>
                        <span className="text-[9px] font-bold text-slate-400 uppercase mt-1 tracking-tighter">ตัวเลข</span>
                      </div>
                      
                      <div className="space-y-1">
                         <div className={`text-lg font-black ${isHot ? 'text-green-600' : 'text-slate-500'}`}>
  ...
  return (
    <div className="space-y-12 animate-in fade-in duration-700 pb-20">
      ...
      <section className="text-center space-y-6">
        <div className="inline-flex items-center justify-center p-5 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-3xl mb-2 shadow-sm">
        ...
        <div className="max-w-2xl mx-auto relative group">
          ...
          <input
            ...
            className="block w-full pl-16 pr-8 py-6 bg-white border-2 border-slate-100 rounded-[2.5rem] shadow-card focus:ring-4 focus:ring-blue-100 focus:border-blue-500 outline-none transition-all text-2xl font-bold placeholder:text-slate-300"
          />
        </div>
      </section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredDreams.map((dream) => (
            <div 
              ...
              className="card-minimal flex flex-col justify-between group hover:border-blue-300 hover:shadow-xl transition-all cursor-pointer border-t-4 border-t-transparent hover:border-t-blue-500 p-8 rounded-[2rem] bg-white relative overflow-hidden"
            >
              ...
              <div className="mt-8 flex items-center justify-between">
                <div className="flex -space-x-3">
                  {dream.luckyNumbers.slice(0, 4).map((n, i) => (
                    <div key={i} className="w-12 h-12 rounded-2xl bg-blue-600 border-4 border-white flex items-center justify-center text-sm font-black text-white z-10 shadow-lg group-hover:scale-110 transition-transform">
                      {n}
                    </div>
                  ))}
                  {dream.luckyNumbers.length > 4 && (
                    <div className="w-12 h-12 rounded-2xl bg-slate-100 border-4 border-white flex items-center justify-center text-[10px] font-black text-slate-400 z-0">
                      +{dream.luckyNumbers.length - 4}
                    </div>
                  )}
                </div>
