...
export async function fetchLatestDrawData(): Promise<Draw | null> {
  // 1. Try to fetch from Official GLO API (Recommended as Rayriffy is archived)
  try {
    const now = new Date();
    ...
    if (result.status && result.response && result.response.result) {
      ...
      const mappedData: Draw = {
        ...
        id: gloData.date.split('-').reverse().join('').slice(0, 4) + (parseInt(gloData.date.split('-')[0]) + 543).toString(),
        prizes: [
    ...
    console.error('GLO API failed:', error);
  }
